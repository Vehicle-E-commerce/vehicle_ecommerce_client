...
const CarlistRandomUser = () => {
  const { carListRandomUser } = useContext(AnnouncementContext);
  
  return (
    <ContainerCarList id="car-list">
      <label>Carros</label>
      <ul>
        {carListRandomUser.length === 0 ? (
          <div>Sem carros amostra</div>
        ) : (
          carListRandomUser.map((car) => (
            <CardsCar
              key={car.id}
...
export default CarlistRandomUser;